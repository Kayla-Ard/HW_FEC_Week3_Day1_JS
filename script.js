...
// sometimes we call it main.js if there's more than one but everything is being imported in this guy
...
//however if we KNOW we may need to reassign our variable use let
let myNewVar = 'Brad';
...
// let doesn't allow variable hoisting aka using a variable before its declaration & assignment 
// console.log(myLet);
// because of variable hoisting, this moves to the top of the scope
let myLet; 
...
// type-guarding. aka do something to the data if its THIS type vs do something else to the data if its THIS type
...
// regular comparison but also STRICT comparison check. 
...
ingredients.push('milk') //push is akin to append in Python
...
// map takes in 1 argument which is the function to run over all items
...
//reduce takes in 1 argument which is function to run over all items
...
//an array has a specific method for iterating called the .forEach()
//the forEach is seen as better practice when iterating over an ARRAY specifically
myBenders.forEach( bender => {
    console.log(bender)
})

// trying to do variable deconstructing with a forEach loop but won't be smooth
...
//spread operator allows you to spread values (from their original container/array) into a new variable/array. 
